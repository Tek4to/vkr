...
def fullname_filter(qs, name, value):
  ...
  vs = value.split()
  qs2 = qs.annotate(full_name=Concat('last_name', V(' '), 'first_name', V(' '), 'fathers_name'))
  for v in vs:
...
class PatientListFilter(filters.FilterSet):
  ...
  fullname = filters.CharFilter(method=fullname_filter,label='ФИО фильтр')
  email__icontains = filters.CharFilter('email','icontains')
  personal_policy__icontains = filters.CharFilter('personal_policy','icontains')
...
# class PatientAndCardUpdateFilter(filters.FilterSet):
#   pass
# class PatientShotsTableFilter(filters.FilterSet):
#   pass

# class MedWorkerPatientsTableFilter(filters.FilterSet):

#   class Meta:
#     model = MedWorkerListView