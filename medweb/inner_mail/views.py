...
class NotificationDynamicsGenerics(generics.ListAPIView):
  ...
  def get_queryset(self):
    ...
    qs = models.NotificationDynamics.objects.filter(
      user__id=user_id
    ).order_by('-update_date'
    ).select_related('mail', 'mail__notification_group', 'mail__details')
    return qs
...
class NotificationsGroupGenerics(generics.ListAPIView):
  ...
  def get_queryset(self):
    if not models.NotificationGroup.objects.filter(
      pk=self.kwargs.get('pk'),
      members__in=[self.request.user]
    ).exists():
      raise PermissionDenied()

    qs =(models.Notification.objects
      .filter(
...
class NotificationsGroupListGenerics(generics.ListAPIView):
  ...
  def get_queryset(self):
    qs = (
      models.NotificationGroup.objects
      .filter(
        members__in=[self.request.user]
      )
      .order_by('-create_date')
